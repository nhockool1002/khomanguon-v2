...
- [x] **Bổ sung 2026-08-11 (ngoài kế hoạch ban đầu):** Gói Subscription/VIP — role RBAC mới `subscription` (gán/gỡ tự động, đa role song song với `member` — không thu hồi quyền $P hiện có), gói tự cấu hình ở `/admin/settings/subscription` (tên/giá/thời hạn/giới hạn tải TỔNG + giới hạn tải THEO NGÀY tuỳ chọn, cả 2 để trống = không giới hạn). Card gói + trạng thái gói đang dùng hiển thị ngay trong **tab "Hồ sơ" hiện có** (không tạo tab riêng) — `components/subscription-plans.tsx` nhúng trong `ProfileTab` (`profile-page.tsx`), dưới `ProfileMessages`. KHÔNG còn ở trang nạp tiền/`wallet-dashboard.tsx` nữa, đã revert.
  ...
  - **Kích hoạt:** mua gói mới luôn thay thế kỳ đang có (đánh dấu EXPIRED, không cộng dồn thời hạn), gán role `subscription` (không đụng `primaryRoleId`/badge). Cron `SubscriptionCronService` (`EVERY_MINUTE`, cùng pattern `sepay-cron.service.ts`) tự gỡ role khi hết hạn — re-check trước khi gỡ để tránh race với người vừa mua gói mới.
  ...
  - **Riêng tư trạng thái Subscription:** permission mới `subscription.view_any` (mặc định gán Super Moderator, Admin có qua `ALL_PERMISSION_KEYS`) — `GET /subscriptions/users/:userId` chỉ trả dữ liệu nếu `viewer.id === userId` hoặc viewer có quyền này (check thủ công trong `subscription.controller.ts`, không dùng `@Permissions()` chuẩn vì cần thêm nhánh "hoặc là chính mình"). Card chỉ hiện với chính chủ hoặc viewer có `subscription.view_any`; người khác (kể cả khách vãng lai) không thấy card này ở tab Hồ sơ. Viewer có quyền xem qua `components/subscription-status-view.tsx` (chỉ đọc, không có nút mua) — chính chủ xem qua `subscription-plans.tsx` (có nút mua).
  - **Test:** 27 unit test riêng cho Subscription (mock Prisma — quota tổng/theo ngày/mỗi lượt tải đều trừ kể cả tải lại, đối soát webhook idempotent/sai số tiền/sai chiều tiền, cron hết hạn có race-check), tổng 44 unit test toàn backend đều pass, + bộ e2e `test/subscription.e2e-spec.ts` chạy Postgres thật (mua gói → webhook → membership/role thật → **webhook trùng lặp không kích hoạt 2 lần** → **regression xác nhận luồng nạp $P hiện có qua CÙNG endpoint webhook vẫn hoạt động đúng** → quota thật qua Postgres → cron hết hạn thật). CI thêm biến `STORAGE_SECRET_KEY` (thiếu từ trước, chặn mọi test đụng secret mã hoá).
