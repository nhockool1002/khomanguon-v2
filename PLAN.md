...
- [x] Chức năng kiểm duyệt (ẩn/xoá/ghim, chặn user spam) cho Moderator — UC08. "Chặn user" dùng `PATCH /users/:id/status` (thu hồi refresh token, enforce ở login/refresh). Bổ sung sau: nút "Khoá / Mở khoá" ngay trong bảng `/quan-tri/nguoi-dung` (trước đây phải gọi API tay để mở khoá lại, giờ không cần nữa).
