...
- [x] Tích hợp Tiptap (WYSIWYG) + upload ảnh trong nội dung — ảnh lưu tạm ổ đĩa cục bộ (volume Docker riêng) cho tới khi có trang cài đặt R2/S3 ở Phase 3. **Cập nhật 2026-08-09:** thử đổi sang CKEditor 5 rồi quay lại Tiptap trong cùng ngày — CKEditor 5 (toolbar đóng gói sẵn, không tự kiểm soát được DOM) bị lỗi nút toolbar cướp focus bàn phím khỏi vùng soạn thảo sau khi click nội dung, vá 3 lần (PR #55/#56/#57) vẫn tái diễn trên production. Tiptap (ProseMirror) cho phép tự viết toolbar bằng React thuần — mọi nút đều `onMouseDown={preventDefault}`, loại bỏ tận gốc cả nhóm bug đó. Nút "Chọn ảnh" mở modal Thư viện Media kiểu WordPress (`media-picker-modal.tsx`, dùng chung với Ảnh đại diện/Ảnh OG), hỗ trợ cả nguồn Local lẫn Cloud R2/S3.
...
- [x] Bổ sung 2026-08-09: nút ẩn "🔑 Admin Get Presigned Link" trên `DownloadBox` — chỉ hiện với user có quyền mới `download.bypass` (mặc định chỉ Admin, gán riêng qua trang Phân quyền cho role khác nếu cần). Gọi `POST /download-links/:linkId/admin-bypass` (`adminBypassUnlock` trong `download-links.service.ts`) — tạo presigned URL như luồng mua thường nhưng KHÔNG trừ $P, KHÔNG ghi `WalletTransaction`/`DownloadGrant`/`DownloadEvent`, không gửi mail thông báo; chỉ ghi `AuditLog` (action `DOWNLOAD_BYPASSED`, permission `audit.view` riêng mới xem được) vì đây là năng lực nhạy cảm dễ bị lạm dụng nếu gán nhầm quyền.
...
- [x] **Bổ sung 2026-08-10:** Modal "Gửi góp ý" (Feedback) — nút mở modal đặt ở Navbar (desktop + mobile), hoạt động cả khi chưa đăng nhập. Model `Feedback` mới (`authorId` optional để hỗ trợ gửi ẩn danh, kèm `name`/`email` tự nhập khi ẩn danh). Backend `POST /feedback` dùng `OptionalJwtAuthGuard` + rate limit riêng (`feedbackCreate`, cấu hình được ở Cài đặt chung, mặc định 5 lượt/giờ/IP). Có email thông báo Admin (`feedbackAdmin`, admin-editable ở Cài đặt Email như các template khác) và trang quản trị `/admin/feedback` (quyền `feedback.manage`, gán mặc định cho Super Moderator) để xem/đánh dấu đã xử lý — mirror đúng pattern `link-reports`.
...
- [ ] Cache Cloudflare Edge cho trang public + asset tĩnh — **Cập nhật 2026-08-10:** phần code đã xong. `HttpCacheInterceptor` (`backend/src/cache/http-cache.interceptor.ts`) giờ set thêm header `Cache-Control: public, s-maxage=<ttl>, stale-while-revalidate=<ttl*2>` cho mọi route đã có `@Cacheable` (public thuần, không route nào đọc `@CurrentUser` — đã rà lại từng chỗ dùng) — CDN đứng trước domain `api.` giờ phục vụ thẳng từ edge được, không chỉ dừng ở Redis. `/uploads/*` (backend `main.ts`) cũng có `public, max-age=31536000, immutable` (an toàn vì tên file luôn `randomUUID()`, không bao giờ ghi đè). Trang public trên frontend (Vercel) vốn đã đủ điều kiện cache CDN qua Next.js ISR có sẵn (mục 4.2 ở trên) — Next.js tự set `Cache-Control: s-maxage=30, stale-while-revalidate` cho các route đó, không cần thêm code. **Còn lại — việc dashboard, không phải code:** bật Cache Rule thật trên Cloudflare cho subdomain `api.` (hướng dẫn chi tiết đã thêm ở [`Deploy_Checklist.md`](Deploy_Checklist.md) mục 2 bước 9b) — cần chủ dự án thao tác tay trên Cloudflare dashboard vì đây là hạ tầng ngoài repo. Domain frontend KHÔNG được bật proxy Cloudflare song song với Vercel (đã ghi rõ ở Deploy_Checklist.md bước 5 mục 4) nên không áp dụng Cache Rule cho domain đó.
...
- [x] **Bổ sung 2026-08-10 (ngoài kế hoạch ban đầu):** Chế độ Bảo trì (Maintenance Mode) — bật/tắt + nội dung tuỳ chỉnh ở `/admin/settings/general` (`GeneralSettings.maintenanceMode`, cùng permission `settings.general` như rate limit/footer). Chặn bằng **redirect thật** (không chỉ render đè) sang route `/bao-tri` (`app/bao-tri/page.tsx`) — `components/site-shell.tsx` (client, thay thế Navbar/{children}/Footer/NewsletterSignup/PostPopup trực tiếp trong `layout.tsx`) dùng `router.replace()` cùng pattern gate quyền đã có ở mọi trang `/admin/*`, áp dụng cho **bất kỳ URL nào** kể cả đã đăng nhập nhưng thiếu quyền. User KHÔNG có quyền `maintenance.bypass` luôn bị đẩy về `/bao-tri` (hình minh hoạ 2 bánh răng + cờ lê tự dựng bằng SVG, `components/maintenance-illustration.tsx` — không phải ảnh upload, không đổi được ngoài code, trang full-screen không kèm Navbar/Footer); user có quyền (gán qua trang Quản lý Permission như mọi quyền khác, Admin luôn có qua `ALL_PERMISSION_KEYS`) thấy site bình thường kèm banner cảnh báo màu vàng ở đầu trang. Các trang `/dang-nhap`, `/dang-ky`, `/quen-mat-khau`, `/dat-lai-mat-khau`, `/xac-minh-email` và chính `/bao-tri` luôn miễn trừ (không thì user có quyền bypass nhưng đang bị đăng xuất sẽ tự khoá luôn chính mình khỏi site, hoặc redirect lặp vô hạn). `/bao-tri` tự redirect về `/` nếu bảo trì đã tắt (link cũ còn lưu). **Giới hạn phạm vi có chủ đích:** chỉ chặn ở FE (trải nghiệm trang web) — KHÔNG chặn API backend (gọi thẳng API vẫn lấy được dữ liệu công khai), vì bản thân trang quản trị (nơi tắt bảo trì) cũng chạy trên cùng backend đó.
- [x] **Bổ sung 2026-08-11 (ngoài kế hoạch ban đầu):** Gói Subscription/VIP — role RBAC mới `subscription` (gán/gỡ tự động, đa role song song với `member` — không thu hồi quyền $P hiện có), gói tự cấu hình ở `/admin/settings/subscription` (tên/giá/thời hạn/giới hạn tải TỔNG + giới hạn tải THEO NGÀY tuỳ chọn, cả 2 để trống = không giới hạn).
  - **Trang Nạp tiền** (`components/subscription-plans.tsx`, nhúng trong `wallet-dashboard.tsx`) — hiển thị card các gói cho **mọi user** (kể cả chưa đăng nhập gói nào), nút "Đăng ký" tự disable khi user đang có 1 kỳ ACTIVE (validate CẢ 2 tầng: FE disable nút + BE `createOrder()` chặn cứng bằng `BadRequestException` nếu gọi thẳng API) — chỉ mở lại được sau khi gói hết hạn hoặc bị Admin thu hồi.
  - **Tab "Hồ sơ"** (`profile-page.tsx`, ngay dưới phần bio, TRƯỚC `ProfileMessages`) — card trạng thái CHỈ xuất hiện SAU KHI user đã đăng ký thành công (ẩn hẳn nếu không có kỳ ACTIVE, không hiện placeholder rỗng), gồm đếm ngược sống động (tick mỗi giây, `components/subscription-status-view.tsx`), số lượt đã tải/còn lại (tổng + theo ngày nếu gói có giới hạn ngày). Chính chủ và viewer có quyền `subscription.view_any` (mặc định Super Moderator, Admin qua `ALL_PERMISSION_KEYS`) đều xem qua component này — người khác/khách vãng lai không thấy card.
  - **Admin revoke:** permission mới `subscription.revoke` (KHÔNG gán mặc định Super Moderator, chỉ Admin qua `ALL_PERMISSION_KEYS` — mạnh hơn hẳn `view_any` nên nhạy cảm hơn) — nút "Thu hồi" ở card Subscription khi xem hồ sơ người khác, gọi `POST /subscriptions/users/:userId/revoke` (`PermissionsGuard` chuẩn, không có nhánh tự-áp-dụng-cho-chính-mình). Đánh dấu membership `REVOKED` (enum mới, phân biệt với `EXPIRED` tự nhiên do cron), gỡ ngay role `subscription`, mất quyền tải free NGAY LẬP TỨC (không đợi cron), ghi `AuditLog` (`SUBSCRIPTION_REVOKED`, cùng cơ chế audit đổi quyền/chỉnh ví tay).
  - **Thanh toán:** luồng SePay RIÊNG, tách hẳn khỏi nạp $P — model `SubscriptionOrder` (mã `SUB######`, khác hẳn `GD######` của `TopupOrder`) + `SubscriptionSepayTransaction` (đối soát idempotent riêng, KHÔNG dùng chung bảng `SepayTransaction`). `sepay-webhook.controller.ts` chỉ THÊM 1 lời gọi sau `sepayService.matchAndCredit()` hiện có (chạy khi không khớp đơn nạp $P) — không sửa 1 dòng nào trong `SepayService`/`sepay.module.ts`; dùng `ModuleRef` (`strict:false`) để lấy `SubscriptionService` mà không tạo vòng phụ thuộc module.
  - **Kích hoạt:** gán role `subscription` (không đụng `primaryRoleId`/badge). Cron `SubscriptionCronService` (`EVERY_MINUTE`, cùng pattern `sepay-cron.service.ts`) tự gỡ role khi hết hạn — re-check trước khi gỡ để tránh race với người vừa mua gói mới. Webhook vẫn giữ nhánh "mua gói mới thay thế kỳ ACTIVE cũ" như 1 lớp phòng thủ (idempotency/race), dù luồng UI bình thường không còn tới được nhánh này nữa do `createOrder()` đã chặn từ trước.
  - **Tải file:** `DownloadLinksController` đổi 1 dòng (`unlock()` -> `unlockSmart()`) — `unlock()` gốc GIỮ NGUYÊN 100% không sửa. `unlockSmart()` kiểm tra quota qua `SubscriptionService.checkFreeDownloadEligibility` — quota đếm theo **TỔNG SỐ LƯỢT TẢI** (`DownloadEvent.count()` theo `subscriptionMembershipId`), **mỗi lần bấm tải đều tính 1 lượt kể cả tải lại đúng link đã tải trước đó** (không có ngoại lệ "link cũ tải lại miễn phí" — khớp đúng cách `unlock()` $P trừ tiền mỗi lần tải). Còn quota thì tải KHÔNG trừ $P (vẫn ghi `DownloadGrant`+`DownloadEvent` đầy đủ để lên đúng danh sách "đã tải"/lịch sử), hết quota thì rơi về `unlock()` ($P) y hệt bình thường — không có khái niệm chặn cứng.
  - **Test:** unit test riêng cho Subscription (mock Prisma — quota tổng/theo ngày/mỗi lượt tải đều trừ kể cả tải lại, đối soát webhook idempotent/sai số tiền/sai chiều tiền, cron hết hạn có race-check, `createOrder()` chặn mua chồng, `revokeMembership()` happy-path/not-found/role-chưa-seed), 49 unit test toàn backend đều pass, + bộ e2e `test/subscription.e2e-spec.ts` chạy Postgres thật (mua gói → webhook → membership/role thật → **webhook trùng lặp không kích hoạt 2 lần** → mua chồng khi đã ACTIVE -> 400 → **regression xác nhận luồng nạp $P hiện có qua CÙNG endpoint webhook vẫn hoạt động đúng** → quota thật qua Postgres → Admin revoke thật (403 cho user thường, thành công cho Admin, revoke lần 2 -> 404) → cron hết hạn thật). CI thêm biến `STORAGE_SECRET_KEY` (thiếu từ trước, chặn mọi test đụng secret mã hoá).
- [x] **Bổ sung 2026-08-18 (ngoài kế hoạch ban đầu):** Slider Carousel + nhập tài liệu docx/html/pdf vào bài viết:
  - **Module Slider** (`backend/src/sliders/`, mirror đúng pattern `widgets/`) — model `Slider` + `SliderSlide` (migration `20260818035925_add_sliders`), cấu hình kiểu bullet (Dots/Numbers/Thumbnails/None) và kiểu chuyển cảnh (Slide/Fade/Zoom/Cube/Coverflow — "Zoom" không có sẵn trong Swiper nên dựng bằng `EffectCreative` scale+opacity tay). Quyền mới `slider.manage` (gán mặc định Super Moderator, giống `widget.manage`). `GET /sliders` + `GET /sliders/:id` công khai (cache 5 phút qua `HttpCacheInterceptor`, cùng cơ chế widgets) — dùng chung cho cả trang Quản lý Slider (`/admin/sliders`) lẫn modal chọn Slider trong trình soạn, không cần endpoint admin riêng vì dữ liệu không nhạy cảm.
  - **Trang Quản lý Slider** (`frontend/src/app/admin/sliders/page.tsx`) — tạo slider từ ảnh có sẵn trong Thư viện Media (tái dùng nguyên `MediaPickerModal`, không viết lại upload), kéo-thả sắp xếp ảnh (`slider-slides-editor.tsx`, cùng pattern `@dnd-kit` với `widget-list-editor.tsx`). Public render `slider-carousel.tsx` dùng thư viện `swiper` (thêm mới) — modules `Autoplay`/`Pagination`/`Thumbs`/`EffectFade`/`EffectCube`/`EffectCoverflow`/`EffectCreative`.
  - **Chèn Slider vào bài viết** — node Tiptap atom mới `sliderEmbed` (`tiptap-slider-node.ts`), nút toolbar "Chèn Slider" mở `slider-picker-modal.tsx` (mirror `media-picker-modal.tsx`). Lưu trong `contentHtml` dưới dạng `<div data-slider-embed data-slider-id="...">` (KHÔNG nhúng dữ liệu slider trực tiếp) — trang công khai `prose-content.tsx` tìm các div này sau khi render, gọi API lấy Slider **mới nhất** rồi mount `SliderCarousel` đè lên bằng `createRoot` (cùng kỹ thuật "post-process DOM sau `dangerouslySetInnerHTML`" đã dùng cho `.img-shine`) — sửa slider ở trang quản lý tự động phản ánh vào mọi bài viết đã chèn, không cần sửa lại từng bài. **Bug đã gặp và sửa:** div gốc còn giữ nguyên class `slider-embed-placeholder` (style riêng cho khung nét đứt trong lúc soạn) sau khi mount — để nguyên gây xung đột layout với nội dung `aspect-video` bên trong (kích thước bị thổi phồng hàng chục nghìn px), phải xoá class này trước khi mount component thật.
  - **Nhập tài liệu** (`backend/src/content-import/`) — 3 endpoint `POST /content-import/{docx,html,pdf}` (quyền `post.create`, cùng bar với `/media`), nút "Nhập tài liệu" trong toolbar trình soạn. docx qua `mammoth` (ảnh nhúng tách qua `convertImage()` callback), html qua `cheerio` (ảnh base64/URL ngoài đều tải về), pdf qua `pdftohtml` (poppler-utils, chế độ `-c -s -noframes` — mỗi trang PDF ra 1 ảnh nền raster hoá + text đặt tuyệt đối đè lên, đây là cách duy nhất giữ layout gần như nguyên bản khi chuyển định dạng trang cố định sang HTML chảy tự do). Toàn bộ output đi qua `sanitize-html` (chặn script/on*, cho phép `style`/`class` để giữ layout pdftohtml). `MediaService` có thêm `uploadBuffer()` (tách từ logic trong `MediaController.upload()`, dùng chung cho cả 3 luồng import) và `MediaModule` export `MediaService` để `ContentImportModule` tái sử dụng. Yêu cầu hệ thống mới: `poppler-utils` (đã thêm vào `backend/Dockerfile` runtime stage, ghi chú cài đặt local ở `CONTRIBUTING.md`).
  - **2 bug thật gặp khi test qua UI (không phát hiện được nếu chỉ test API bằng curl):** (1) backend trả URL ảnh **tương đối** cho lưu trữ Local (đúng quy ước sẵn có) — nếu chèn thẳng vào `insertContent()` mà không ghép `NEXT_PUBLIC_API_URL` như mọi nơi khác đã làm (`toAbsoluteUploadUrl`), ảnh sẽ 404 vì trình duyệt tự resolve theo origin frontend thay vì backend; đã thêm bước `absolutizeImageSrc()` (parse DOM, rewrite `src`) trước khi chèn. (2) chèn 2 tài liệu liên tiếp (không thao tác gì ở giữa) làm ảnh của lượt chèn TRƯỚC biến mất — nguyên nhân: sau `insertContent()` một node atom (ảnh/slider), ProseMirror để lại `NodeSelection` NGAY TRÊN node đó thay vì dời con trỏ ra sau, nên lệnh `insertContent`/`insertSliderEmbed` kế tiếp thay thế đúng node đang được chọn. Sửa bằng cách dời selection về cuối tài liệu (`setTextSelection(editor.state.doc.content.size)`) bằng 1 lệnh `.run()` **riêng** sau lệnh insert (không gộp chung 1 chain — `editor.state` bên trong cùng 1 chain vẫn là state TRƯỚC khi chain chạy).
