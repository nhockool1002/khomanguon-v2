...
- [x] **Bổ sung 2026-08-11 (ngoài kế hoạch ban đầu):** Gói Subscription/VIP — role RBAC mới `subscription` (gán/gỡ tự động, đa role song song với `member` — không thu hồi quyền $P hiện có), gói tự cấu hình ở `/admin/settings/subscription` (tên/giá/thời hạn/giới hạn tải TỔNG + giới hạn tải THEO NGÀY tuỳ chọn, cả 2 để trống = không giới hạn). Card gói hiển thị ở trang nạp tiền (`components/subscription-plans.tsx`, nhúng trong `wallet-dashboard.tsx`).
  ...
  - **Tải file:** `DownloadLinksController` đổi 1 dòng (`unlock()` -> `unlockSmart()`) — `unlock()` gốc GIỮ NGUYÊN 100% không sửa. `unlockSmart()` kiểm tra quota qua `SubscriptionService.checkFreeDownloadEligibility` (đếm SỐ LINK PHÂN BIỆT đã tải qua `DownloadEvent.subscriptionMembershipId`, không đếm số lượt bấm — tải lại link đã mở khoá luôn miễn phí, không trừ thêm quota); còn quota thì tải KHÔNG trừ $P (vẫn ghi `DownloadGrant`+`DownloadEvent` đầy đủ để lên đúng danh sách "đã tải"/lịch sử), hết quota thì rơi về `unlock()` ($P) y hệt bình thường — không có khái niệm chặn cứng.
  - **Test:** 44 unit test (mock Prisma — quota tổng/theo ngày/tải lại link cũ, đối soát webhook idempotent/sai số tiền/sai chiều tiền, cron hết hạn có race-check) + bộ e2e `test/subscription.e2e-spec.ts` chạy Postgres thật (mua gói → webhook → membership/role thật → **webhook trùng lặp không kích hoạt 2 lần** → **regression xác nhận luồng nạp $P hiện có qua CÙNG endpoint webhook vẫn hoạt động đúng** → quota thật qua Postgres → cron hết hạn thật). CI thêm biến `STORAGE_SECRET_KEY` (thiếu từ trước, chặn mọi test đụng secret mã hoá).
