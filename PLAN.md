...
- [x] **Bổ sung 2026-08-10 (ngoài kế hoạch ban đầu):** Chế độ Bảo trì (Maintenance Mode) — bật/tắt + nội dung tuỳ chỉnh ở `/admin/settings/general` (`GeneralSettings.maintenanceMode`, cùng permission `settings.general` như rate limit/footer). Chặn ở tầng FE (`components/maintenance-gate.tsx`, bọc toàn bộ `layout.tsx`): user KHÔNG có quyền `maintenance.bypass` chỉ thấy trang bảo trì full-screen (hình minh hoạ 2 bánh răng + cờ lê tự dựng bằng SVG, `components/maintenance-illustration.tsx` — không phải ảnh upload, không đổi được ngoài code); user có quyền (gán qua trang Quản lý Permission như mọi quyền khác, Admin luôn có qua `ALL_PERMISSION_KEYS`) thấy site bình thường kèm banner cảnh báo màu vàng ở đầu trang. Các trang `/dang-nhap`, `/dang-ky`, `/quen-mat-khau`, `/dat-lai-mat-khau`, `/xac-minh-email` luôn miễn trừ (không thì user có quyền bypass nhưng đang bị đăng xuất sẽ tự khoá luôn chính mình khỏi site). **Giới hạn phạm vi có chủ đích:** chỉ chặn ở FE (trải nghiệm trang web) — KHÔNG chặn API backend (gọi thẳng API vẫn lấy được dữ liệu công khai), vì bản thân trang quản trị (nơi tắt bảo trì) cũng chạy trên cùng backend đó.
