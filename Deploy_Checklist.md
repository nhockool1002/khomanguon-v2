...
- [ ] Quyết định 2 domain con: ví dụ `khomanguon.vn` (frontend) và `kmn2api.nhutnm.id.vn` (backend) — dùng đúng 2 tên này trong các bước dưới, đổi lại nếu bạn muốn tên khác
...
5. **Chạy migration + seed** (lần đầu, tạo bảng + 4 role mặc định):
   ...
   ```
6. **Tạo site** trong aaPanel: **Website → Add site** → domain `kmn2api.nhutnm.id.vn` (không cần PHP).
...
11. **Test nhanh** (từ máy bất kỳ):
    ```bash
    curl https://kmn2api.nhutnm.id.vn/health
...
3. **Environment Variables** (áp dụng cho cả Production + Preview):
   ```
   NEXT_PUBLIC_API_URL=https://kmn2api.nhutnm.id.vn
   ```
4. Bấm **Deploy**.
