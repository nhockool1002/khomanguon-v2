...
function deriveKey(context: string): Buffer {
  ...
  if (!secret) {
    throw new Error('Thiếu biến môi trường STORAGE_SECRET_KEY để mã hoá secret');
  }
  return scryptSync(secret, context, 32);
}

export function encryptSecret(plaintext: string, context: string = DEFAULT_CONTEXT): string {
...
export function decryptSecret(encrypted: string, context: string = DEFAULT_CONTEXT): string {
